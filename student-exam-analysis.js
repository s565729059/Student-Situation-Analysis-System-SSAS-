...
async function seaGenerateAnalysis() {
    ...
    const prompt = `你是一位经验丰富的${examSubject}教师，请根据以下信息，对${studentName}同学的${examName}做题情况做一个深入分析。
...
5. 结合${examSubject}学科特色进行分析，比如${examSubject === '数学' ? '计算能力、逻辑思维、解题规范' : examSubject === '英语' ? '词汇积累、语法运用、阅读理解' : examSubject === '语文' ? '阅读理解、写作表达、基础知识' : '学科核心能力'}等方面
6. 分析要具体到知识点，不要泛泛而谈`;
...
async function seaGenerateBeautifiedHtml() {
    ...
    const prompt = `你是一位世界顶尖的排版大师和前端工程师，擅长将文字内容转化为视觉震撼的HTML页面。请将以下考情分析内容转换为一份极其精美、现代化的HTML报告。

【内容结构要求】
一、封面：青岛睿花苑·${examSubject}考情分析报告（大标题居中，简约大气，必须包含"青岛睿花苑"字样，无版本标识）
二、学生信息卡：姓名（${studentName}）、年级、考试名称、成绩等关键信息，使用卡片式布局
三、分析内容：将考情分析文字按逻辑分段，使用卡片/模块化布局，每段一个小标题
四、总结与鼓励：突出鼓励性语言，使用醒目的样式
五、装饰元素：适当使用CSS绘制的装饰线条、渐变色块、图标等
六、配色方案：根据${examSubject}学科特色选择配色（${examSubject === '数学' ? '蓝色系，理性严谨' : examSubject === '英语' ? '绿色系，活力开放' : examSubject === '语文' ? '红棕色系，文化底蕴' : '紫色系，智慧优雅'}）
七、页脚：必须包含以下内容（无年份）：
    - 主文字："小睿同学·智能考情分析系统"
        ...
        const response = await fetch(SEA_ZHIPU_API_URL, {
            ...
            body: JSON.stringify({
                ...
                messages: [
                    {
                        role: 'system',
                        content: '你是一位世界顶级的HTML报告设计师和前端工程师。你擅长生成极其精美、现代化、数据可视化丰富的考情分析报告。你必须输出完整、可运行的HTML代码，所有内容必须完整输出，绝不能截断。图表必须使用内联SVG或CSS绘制，不依赖外部JS库初始化。【强制品牌要求】1. 报告大标题必须包含"青岛睿花苑"字样，格式为"青岛睿花苑·XXX考情分析报告"；2. 页脚必须包含三行信息：主文字"小睿同学·智能考情分析系统"、版权信息"©版权所有·青岛睿花苑教育科技有限公司"、企业标语"打造最适合人才发展的教育平台，为所到地区带去最优质的教育"。以上品牌信息为强制要求，不可省略。'
                    },
