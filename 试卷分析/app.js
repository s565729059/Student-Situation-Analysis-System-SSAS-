...
const elements = {
    ...
    versionCards: document.querySelectorAll('.version-card'),
...
function processFile(file) {
    ...
    if (fileExtension === '.pdf') {
        ...
        elements.fileIcon.className = 'fas fa-file-pdf text-3xl text-red-500 mr-4';
    } else {
        elements.pdfWarning.classList.add('hidden');
        elements.fileIcon.className = 'fas fa-file-word text-3xl text-blue-500 mr-4';
    }
...
function displayAnalysisResults() {
    ...
    elements.versionBadge.className = `px-3 py-1 rounded-full text-sm font-medium ${
        state.selectedVersion === 'teaching' ? 'bg-blue-100 text-blue-800' : 'bg-pink-100 text-pink-800'
    }`;
...
function formatAnalysisContent(content) {
    return content
        .replace(/^#{3}\s+(.*)/gm, '<h4 class="font-bold text-gray-800 mt-4 mb-2">$1</h4>')
        .replace(/^#{2}\s+(.*)/gm, '<h3 class="font-bold text-lg text-gray-800 mt-4 mb-2">$1</h3>')
        .replace(/^#{1}\s+(.*)/gm, '<h2 class="font-bold text-xl text-gray-800 mt-4 mb-2">$1</h2>')
        .replace(/\*\*(.*?)\*\*/g, '<strong>$1</strong>')
        .replace(/\*(.*?)\*/g, '<em>$1</em>')
        .replace(/^-\s+(.*)/gm, '<li class="ml-4 mb-1">$1</li>')
        .replace(/^(\d+)\.\s+(.*)/gm, '<li class="ml-4 mb-1"><span class="font-medium">$1.</span> $2</li>')
        .replace(/\n/g, '<br>');
}
